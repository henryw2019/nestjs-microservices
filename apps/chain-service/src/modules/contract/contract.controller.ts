...
export class ContractController {
    ...
    @ApiOkResponse({ type: ContractResponseDto })
    findOne(@Param('id') id: string, @Query('withAbi') withAbi?: string): Promise<ContractResponseDto> {
        const includeAbi = typeof withAbi === 'string' ? ['true', '1', 'yes'].includes(withAbi.toLowerCase()) : false;

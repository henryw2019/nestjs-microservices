...
export class ContractService {
    ...
    async update(id: string, dto: UpdateContractDto): Promise<ContractResponseDto> {
        ...
        return this.toResponse(
            refreshed,
            dto.abi ? await this.loadAbi(refreshed.abiFile) : undefined,
        );
    }
    ...
    async execute(
        ...
        try {
            ...
            this.logger.log(
                `Executed ${dto.functionName} on ${contract.address}; tx hash: ${tx.hash}`,
            );
            return response;
        } catch (error) {
            if (error instanceof NotFoundException) {
    ...
    private resolveCallable(
        ...
        const candidates = [fragment.format('full'), fragment.format('minimal'), fragment.name];
    ...
    private async buildWriteResult(
        ...
        const transaction = {
            ...
            chainId:
                typeof tx.chainId === 'number' ? tx.chainId : Number(process.env.CHAIN_ID || 0),
        };

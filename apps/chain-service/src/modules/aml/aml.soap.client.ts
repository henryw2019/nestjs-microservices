...
export class AmlSoapClient {
    ...
    async realtimeScan(payload: AmlScanRequestPayload): Promise<AmlScanResponsePayload> {
        ...
        const method: ISoapMethod | undefined =
            (client as any)['RealtimeScan'] || (client as any)['realtimeScan'];
        if (!method) throw new Error('SOAP method RealtimeScan not found in WSDL');

        try {
            const [result] = await ((method as any) + 'Async' in client
                ? (client as any).RealtimeScanAsync(payload)
                : method(payload));

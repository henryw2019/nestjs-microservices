...
import {
    AmlDecision,
    AmlPartyInfo,
    AmlScanRequestPayload,
    AmlScanResponsePayload,
} from './aml.types';
...
export class AmlService {
    ...
    async scanTransfer(input: {
        ...
        await (this.db as any).amlScanEvent.create({
            data: { amlScanId: scan.id, type: 'REQUEST', payload: requestPayload as any },
        });
    ...
    async persistResponse(scanId: string, response: AmlScanResponsePayload) {
        await this.db.$transaction([
            ...
            (this.db as any).amlScanEvent.create({
                data: { amlScanId: scanId, type: 'RESPONSE', payload: response as any },
            }),
        ]);
    }
    ...
    shouldBlock(decision: AmlDecision): boolean {
        ...
        if (this.enforcement === 'block_on_review')
            return decision === 'FAILED' || decision === 'REVIEW';
    ...
    async pollPendingOnce(limit = 20) {
        const pending = await (this.db as any).amlScan.findMany({
            where: { status: { in: ['PENDING', 'REVIEW'] }, correlationId: { not: null } },
            take: limit,
        ...
        for (const s of pending) {
            try {
                ...
                this.logger.warn(
                    `Polling failed for scan ${s.id}: ${(err as any)?.message || err}`,
                );
            }
        }
    }
